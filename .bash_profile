...
if [[ -t 2 ]]; then
	...
	for ENTRY in ${ARRAY_MANPATH[@]}; do
		[[ -d ${ENTRY} ]] && MANPATH="${MANPATH}${MANPATH+:}${ENTRY}" && _bash_env_loading
	done
	export MANPATH
	...
	for ENTRY in ${ARRAY_NODE_PATH[@]}; do
		[[ -d ${ENTRY} ]] && NODE_PATH="${NODE_PATH}${NODE_PATH+:}${ENTRY}" && _bash_env_loading
	done
	export NODE_PATH
	...
	for ENTRY in ${ARRAY_MIBDIRS[@]}; do
		[[ -d ${ENTRY} ]] && MIBDIRS="${MIBDIRS}${MIBDIRS+:}${ENTRY}" && _bash_env_loading
	done
	export MIBDIRS
	export MIBS="ALL"
	...
	for ENTRY in ${ARRAY_MKDIR[@]}; do
		[[ ! -d ${ENTRY} ]] && mkdir -p ${ENTRY} && _bash_env_loading
	done

	# Remove files if present
	for ENTRY in ${ARRAY_RM[@]}; do
		[[ -e ${ENTRY} ]] && rm -rf ${ENTRY} && _bash_env_loading
	done

	# Touch files
	for ENTRY in ${ARRAY_TOUCH[@]}; do
		[[ ! -e ${ENTRY} ]] && touch ${ENTRY} && _bash_env_loading
	done
...
if hash facter; then
	export DEFROUTE_NIC="$(facter networking.primary)"
	export HOST_IP="$(     facter networking.interfaces.${DEFROUTE_NIC}.ip)"
...
for ENTRY in ${ARRAY_SOURCE[@]}; do
	[[ -s ${ENTRY} ]] && . ${ENTRY} && _bash_env_loading # Output loading message
done
